...
// Heap xl_info op values (PG18: access/heapam_xlog.h).
//
// The opcode occupies XLOG_HEAP_OPMASK; the top bit is the independent
// XLOG_HEAP_INIT_PAGE flag.  Decoding the whole high nibble as one value, as
// this used to, mis-reads every "+INIT" record -- HOT_UPDATE+INIT is 0xC0,
// not the 0xB0 that was listed here.
...
impl HeapOp {
    ...
    pub fn is_init_page(info: u8) -> bool {
        (info & XLOG_HEAP_INIT_PAGE) != 0
    }

    /// The identifier pg_waldump prints, e.g. "HOT_UPDATE+INIT".
...
pub fn decode_heap_insert_flags(flags: u8) -> String {
    let mut p = Vec::new();
    if flags & (1 << 0) != 0 {
        p.push("ALL_VISIBLE_CLEARED");
    }
    if flags & (1 << 1) != 0 {
        p.push("LAST_IN_MULTI");
    }
    if flags & (1 << 2) != 0 {
        p.push("IS_SPECULATIVE");
    }
    if flags & (1 << 3) != 0 {
        p.push("CONTAINS_NEW_TUPLE");
    }
    if flags & (1 << 4) != 0 {
        p.push("ON_TOAST_RELATION");
    }
    if flags & (1 << 5) != 0 {
        p.push("ALL_FROZEN_SET");
    }
    if p.is_empty() {
        format!("0x{:02x}", flags)
    } else {
        format!("0x{:02x} ({})", flags, p.join(" | "))
    }
}
...
pub fn decode_infobits(b: u8) -> String {
    let mut p = Vec::new();
    if b & 0x01 != 0 {
        p.push("XMAX_IS_MULTI");
    }
    if b & 0x02 != 0 {
        p.push("XMAX_LOCK_ONLY");
    }
    if b & 0x04 != 0 {
        p.push("XMAX_EXCL_LOCK");
    }
    if b & 0x08 != 0 {
        p.push("XMAX_KEYSHR_LOCK");
    }
    if b & 0x10 != 0 {
        p.push("KEYS_UPDATED");
    }
...
fn decode_heap_delete_flags(flags: u8) -> String {
    ...
    if flags & (1 << 1) != 0 {
        p.push("CONTAINS_OLD_TUPLE");
    }
    if flags & (1 << 2) != 0 {
        p.push("CONTAINS_OLD_KEY");
    }
    if flags & (1 << 3) != 0 {
        p.push("IS_SUPER");
    }
    if flags & (1 << 4) != 0 {
        p.push("IS_PARTITION_MOVE");
    }
    if p.is_empty() {
        format!("0x{:02x}", flags)
    } else {
...
fn decode_heap_update_flags(flags: u8) -> String {
    let mut p = Vec::new();
    if flags & (1 << 0) != 0 {
        p.push("OLD_ALL_VISIBLE_CLEARED");
    }
    if flags & (1 << 1) != 0 {
        p.push("NEW_ALL_VISIBLE_CLEARED");
    }
    if flags & (1 << 2) != 0 {
        p.push("CONTAINS_OLD_TUPLE");
    }
    ...
    if flags & (1 << 5) != 0 {
        p.push("PREFIX_FROM_OLD");
    }
    if flags & (1 << 6) != 0 {
        p.push("SUFFIX_FROM_OLD");
    }
    if p.is_empty() {
        format!("0x{:02x}", flags)
    } else {
...
fn decode_t_infomask(mask: u16) -> String {
    ...
    if mask & 0x0100 != 0 {
        p.push("XMIN_COMMITTED");
    }
    if mask & 0x0200 != 0 {
        p.push("XMIN_INVALID");
    }
    if mask & 0x0400 != 0 {
        p.push("XMAX_COMMITTED");
    }
    if mask & 0x0800 != 0 {
        p.push("XMAX_INVALID");
    }
...
pub fn describe_heap_main(info: u8, main: &[u8], _record: &WALRecordInfo) -> Vec<String> {
    ...
    match op {
        ...
        HeapOp::Delete => {
            ...
            lines.push(format!(
                "  flags:        {}",
                decode_heap_delete_flags(flags)
            ));
        ...
        HeapOp::Update | HeapOp::HotUpdate => {
            ...
            lines.push(format!(
                "  old_infobits_set: {}",
                decode_infobits(old_infobits)
            ));
            lines.push(format!(
                "  flags:            {}",
                decode_heap_update_flags(flags)
            ));
        ...
        HeapOp::Confirm => {
            let offnum = r.read_u16_le().unwrap_or(0);
            lines.push(format!(
                "  offnum:  {} (speculative insertion confirmed)",
                offnum
            ));
        }
    ...
    match op {
        ...
        HeapOp::Update | HeapOp::HotUpdate => {
            if block_idx == 0 {
                ...
                if let Some(main) = record.main_data()
                    && main.len() >= 8
                {
                    let flags = main[7];
                    if flags & (1 << 5) != 0 {
...
mod tests {
    ...
    #[test]
    fn heap_opcodes_match_postgres() {
        // access/heapam_xlog.h
        assert_eq!(HeapOp::from_xl_info(0x00), HeapOp::Insert);
        assert_eq!(HeapOp::from_xl_info(0x10), HeapOp::Delete);
        assert_eq!(HeapOp::from_xl_info(0x20), HeapOp::Update);
        assert_eq!(HeapOp::from_xl_info(0x30), HeapOp::Truncate);
        assert_eq!(HeapOp::from_xl_info(0x40), HeapOp::HotUpdate);
        assert_eq!(HeapOp::from_xl_info(0x50), HeapOp::Confirm);
        assert_eq!(HeapOp::from_xl_info(0x60), HeapOp::Lock);
        assert_eq!(HeapOp::from_xl_info(0x70), HeapOp::Inplace);
    }

    /// XLOG_HEAP_INIT_PAGE is an independent flag; decoding the whole high
    /// nibble used to turn HOT_UPDATE+INIT (0xC0) into "Unknown".
    #[test]
    fn init_page_flag_is_independent_of_the_opcode() {
        assert_eq!(HeapOp::from_xl_info(0x80), HeapOp::Insert);
        assert_eq!(HeapOp::from_xl_info(0xA0), HeapOp::Update);
        assert_eq!(HeapOp::from_xl_info(0xC0), HeapOp::HotUpdate);

        assert_eq!(HeapOp::identify(0x00), "INSERT");
        assert_eq!(HeapOp::identify(0x80), "INSERT+INIT");
        assert_eq!(HeapOp::identify(0x20), "UPDATE");
        assert_eq!(HeapOp::identify(0xA0), "UPDATE+INIT");
        assert_eq!(HeapOp::identify(0x40), "HOT_UPDATE");
        assert_eq!(HeapOp::identify(0xC0), "HOT_UPDATE+INIT");

        // The XLR_INFO_MASK bits belong to the record header, not the rmgr.
        assert_eq!(HeapOp::identify(0x0F), "INSERT");
    }
}
