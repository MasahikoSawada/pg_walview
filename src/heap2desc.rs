...
use crate::heapdesc::{HeapOp, XLOG_HEAP_OPMASK, decode_heap_insert_flags, decode_infobits};
use crate::walmisc::{Reader, lsn_format};
...
impl Heap2Op {
    ...
    /// The identifier pg_waldump prints, e.g. "MULTI_INSERT+INIT".
    pub fn identify(info: u8) -> String {
        let op = Self::from_xl_info(info);
        if HeapOp::is_init_page(info) {
            format!("{}+INIT", op)
        } else {
            op.to_string()
        }
...
fn fmt_prune_flags(flags: u8) -> String {
    let mut p = Vec::new();
    if flags & (1 << 1) != 0 {
        p.push("IS_CATALOG_REL");
    }
    if flags & (1 << 2) != 0 {
        p.push("CLEANUP_LOCK");
    }
    if flags & (1 << 3) != 0 {
        p.push("HAS_CONFLICT_HORIZON");
    }
    if flags & (1 << 4) != 0 {
        p.push("HAS_FREEZE_PLANS");
    }
    if flags & (1 << 5) != 0 {
        p.push("HAS_REDIRECTIONS");
    }
    if flags & (1 << 6) != 0 {
        p.push("HAS_DEAD_ITEMS");
    }
    if flags & (1 << 7) != 0 {
        p.push("HAS_NOW_UNUSED_ITEMS");
    }
    if p.is_empty() {
        format!("0x{:02x}", flags)
    } else {
...
pub fn describe_heap2_block_data(_info: u8, _block_idx: usize, data: &[u8]) -> Vec<String> {
    vec![format!(
        "  block data: {} bytes (multi-insert tuple data)",
        data.len()
    )]
}
...
mod tests {
    ...
    #[test]
    fn heap2_opcodes_match_postgres() {
        assert_eq!(Heap2Op::from_xl_info(0x00), Heap2Op::Rewrite);
        assert_eq!(Heap2Op::from_xl_info(0x10), Heap2Op::PruneOnAccess);
        assert_eq!(Heap2Op::from_xl_info(0x20), Heap2Op::PruneVacuumScan);
        assert_eq!(Heap2Op::from_xl_info(0x30), Heap2Op::PruneVacuumCleanup);
        assert_eq!(Heap2Op::from_xl_info(0x40), Heap2Op::Visible);
        assert_eq!(Heap2Op::from_xl_info(0x50), Heap2Op::MultiInsert);
        assert_eq!(Heap2Op::from_xl_info(0x60), Heap2Op::LockUpdated);
        assert_eq!(Heap2Op::from_xl_info(0x70), Heap2Op::NewCid);

        assert_eq!(Heap2Op::identify(0x50), "MULTI_INSERT");
        assert_eq!(Heap2Op::identify(0xD0), "MULTI_INSERT+INIT");
    }
}
