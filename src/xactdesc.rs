...
fn decode_xact_xinfo(xinfo: u32) -> String {
    ...
    if xinfo & (1 << 8) != 0 {
        p.push("HAS_DROPPED_STATS");
    }
    if xinfo & (1 << 29) != 0 {
        p.push("APPLY_FEEDBACK");
    }
    if xinfo & (1 << 30) != 0 {
        p.push("UPDATE_RELCACHE_FILE");
    }
    if xinfo & (1 << 31) != 0 {
        p.push("FORCE_SYNC_COMMIT");
    }
    if p.is_empty() {
        format!("0x{:08x}", xinfo)
    } else {
        format!("0x{:08x} ({})", xinfo, p.join(" | "))
    }
}
...
pub fn describe_xact_main(info: u8, main: &[u8]) -> Vec<String> {
    ...
    match op {
        XactOp::Commit | XactOp::CommitPrepared => {
            ...
            if has_info && let Some(xinfo) = r.read_u32_le() {
